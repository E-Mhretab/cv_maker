...
class CvManageController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Cv::leftJoin('cv_metadata as m', 'cv.id', '=', 'm.cv_id')
            ->select('cv.id','cv.name','cv.profile_summary','cv.user_id','cv.email','m.created_at','m.template_type','m.is_public','m.published_at');
        ...
        $query->orderByDesc('m.created_at')->orderByDesc('cv.id');
    ...
    {
        $request->validate(['template_type' => 'required|in:nathan,esey']);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            AuditLogger::logCV('CREATE', $cv->id, Auth::id(), null, $request->all());
            ...
            return redirect()->route('manage.cv_preview', ['id' => $cv->id])
                ->with('message','CV creado exitosamente!')
                ->with('type','success');
        } catch (\Exception $e) {
            DB::rollback();
            return back()->with('message','Error creando CV: '.$e->getMessage())
                ->with('type','error')
                ->withInput();
        }
    }
    ...
    public function renderCVTemplate($id)
    {
        ...
        if (!in_array($templateType,$allowedTemplates)) $templateType='nathan';
        $templateView = "templates.template_{$templateType}";
        if (!view()->exists($templateView)) {
            $templateView='templates.template_nathan';
            if (!view()->exists($templateView)) abort(404,"Template file not found: {$templateView}");
        }

        return view($templateView, compact('cv','workExperiences','education','hobbies','skills','languages'));
    }

    protected function getTemplateType($cv)
    {
        if (isset($cv['template_type'])) {
            if (is_numeric($cv['template_type'])) return getTemplateName($cv['template_type']);
    ...
    public function exportPdf($id)
    {
        ...
        if (!view()->exists($pdfView)) abort(404,"PDF template not found");

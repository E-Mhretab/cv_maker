...
<head>
    ...
    <style>
        ...
        .search-section { background: #f8f9fa; border-radius: 10px; padding: 1.5rem; margin-bottom: 2rem; }
...
<body class="bg-light">
                ...
                @if ($userContext['is_logged_in'])
                    <div class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">
                            <i class="fas fa-user me-1"></i>{{ $userContext['user']->username }}
                            <span class="badge bg-light text-dark ms-1">{{ ucfirst($userContext['role']) }}</span>
                        </a>
    ...
    <div class="container my-5" style="padding-top: 120px;">
        ...
        <div class="row mb-4">
            <div class="col-md-3 mb-3">
                <div class="stats-card p-3 text-center">
                    <h3 class="h4 mb-1">{{ $cvs->total() }}</h3>
                    <p class="mb-0">Total CVs</p>
                </div>
            </div>
            <div class="col-md-3 mb-3">
                <div class="stats-card p-3 text-center">
                    <h3 class="h4 mb-1">{{ $cvs->filter(fn($cv) => $cv->template_type == 1)->count() }}</h3>
                    <p class="mb-0">Nathan Templates</p>
                </div>
            </div>
            <div class="col-md-3 mb-3">
                <div class="stats-card p-3 text-center">
                    <h3 class="h4 mb-1">{{ $cvs->filter(fn($cv) => $cv->template_type == 2)->count() }}</h3>
                    <p class="mb-0">Esey Templates</p>
                </div>
            </div>
            <div class="col-md-3 mb-3">
                <div class="stats-card p-3 text-center">
                    <h3 class="h4 mb-1">{{ $cvs->filter(fn($cv) => $cv->created_at && date('Y-m-d', strtotime($cv->created_at)) === date('Y-m-d'))->count() }}</h3>
        ...
        <div class="search-section">
            <form method="GET" action="{{ route('manage.cvs.index') }}" class="row g-3">
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="search" name="search" value="{{ $search }}" placeholder="Search by name or summary...">
                    </div>
                </div>
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="template" name="template">
                        ...
                        <option value="1" {{ $templateFilter === '1' ? 'selected' : '' }}>Nathan Template</option>
                        <option value="2" {{ $templateFilter === '2' ? 'selected' : '' }}>Esey Template</option>
                        <option value="3" {{ $templateFilter === '3' ? 'selected' : '' }}>Mirian Template</option>
                    </select>
                </div>
                <div class="col-md-2">
                    ...
                    <div class="cv-card card h-100">
                        <div class="card-body">
                            ...
                            <p class="card-text text-muted small mb-3">
                                {{ substr($cv->profile_summary, 0, 100) }}
                                @if (strlen($cv->profile_summary) > 100)...@endif
                            </p>
                            
                            <div class="text-muted small mb-3">
                                <i class="fas fa-calendar me-1"></i>
                                Created: {{ $cv->created_at ? date('M j, Y', strtotime($cv->created_at)) : 'Date not available' }}
                            </div>
                            
                            <div class="action-buttons">
                                <a href="{{ route('manage.cvs.show', $cv->id) }}" 
                                   class="btn btn-outline-primary btn-sm" title="Preview">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('manage.cvs.edit', $cv->id) }}" 
                                   class="btn btn-outline-warning btn-sm" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                @if ($cv->is_public ?? false)
                                    <a href="{{ url('manage/cvs/publish', ['id' => $cv->id, 'action' => 'unpublish']) }}" 
                                ...
                                @else
                                    <a href="{{ url('manage/cvs/publish', ['id' => $cv->id, 'action' => 'publish']) }}" 
